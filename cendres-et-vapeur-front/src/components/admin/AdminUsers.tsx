...
export default function AdminUsersStatic() {
  ...
  const [users, setUsers] = useState<UserOut[]>(usersSeed);
    ...
    try {
      ...
      if (mode === "create") {
        ...
        const created: UserOut = {
          ...
          role: (form.role?.trim() || "user").toLowerCase(),
          avatar_url: form.avatar_url?.trim() ? form.avatar_url : null,
          biography: form.biography?.trim() ? form.biography : null,
        };
