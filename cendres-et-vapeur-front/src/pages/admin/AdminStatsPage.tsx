...
export default function AdminStatsPage() {
  ...
  const totalOrders = ordersSeed.length;
  ...
  const top5Products = useMemo(() => {
    return [...productsSeed].sort((a, b) => b.price - a.price).slice(0, 5);
  }, []);
    ...
    <div className="adminStatsPage">
      ...
      <section className="statsGrid">
        ...
        <div className="statsCard">
          ...
          <p className="statsValue">{totalUsers}</p>
          <p className="statsHint">Comptes existants</p>
        </div>

        <div className="statsCard">
          <p className="statsLabel">Revenu total</p>
          <p className="statsValue">{formatEUR(totalRevenue)}</p>
          ...
          <div className="topTable">
            ...
            {top5Products.map((p) => (
              <div className="topRow" key={p.id}>
                ...
                <span className="topPrice">{formatEUR(p.price)}</span>
              </div>
            ))}
          </div>
        </div>
